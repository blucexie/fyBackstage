/**
 * Created by funinbook on 2016/11/2.
 */
$(function () {
    //左侧栏切换
    $('.ol1').click(function () {
    ...
    //欢迎名字
    funinbookSecurityAjax("/admin/getUserInfo", null, "POST", true, executeUserInfo);
  ...
  /*  if (getUserInfo() == null){
    ...
    $('.out').click(function () {
        clearSessionStorage();
        // 退出登录
        funinbookUnsafeAjax("/admin/logout", null, "POST", true, executeLogOut);
    })
});
...
function executeUserInfo(data) {
    ...
    // 填充页面数据
    $('.companyName').text(data.msg.userId);
    // 清除用户数据
    this.clearUserInfo();
    // 向session中保存数据
